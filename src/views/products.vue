<template>
   <body>
     ...
     <div class="products">
         <main>
              <div v-for="product in products" :key="product.id" class="azil_product section">
              <Azil_product @emitOpenModalOrder="openOrderModal" :product="product"/>
             </div>
        </main>
     </div>
     <Complete/> 
     <Footer/>
     <transition enter-active-class="animate__animated animate__fadeIn" leave-active-class="animate__animated animate__fadeOut animate__delay-.5s">
    <Checkout @closeOrderModal="popOffModal" :product="checkData" v-if="showCheckout"/>   
     </transition>
    ...
    main{
     ...
     color: rgb(243, 243, 243);
      h1{
        ...
        width:80%;
        margin: 0 auto;
        font-size: 5em;color: rgb(255, 255, 255);
      }
    }
  }
@keyframes animate1 {
